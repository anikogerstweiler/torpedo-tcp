...
public class MessageValidator extends ChannelDuplexHandler {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object actualMessage) throws Exception {
        ...
        if ((actualMessage instanceof Sunk || actualMessage instanceof Hit || actualMessage instanceof Miss) && !(lastMessageSent instanceof Fire)) {
    ...
    protected void verifyFireMessage(Object actualMessage) {
        if (actualMessage instanceof Fire
                && !(lastMessageSent instanceof Size || lastMessageGet instanceof Hit || lastMessageGet instanceof Sunk || lastMessageGet instanceof Miss)) {
            signalError(actualMessage);
        }
    }
    ...
    protected void signalError(Object actualMessage) {
        throw new IllegalArgumentException(actualMessage + " message received, when last message got: " + lastMessageGet + ", last message sent: "
                + lastMessageSent);
    }
